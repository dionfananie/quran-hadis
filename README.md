...
- Reading tools on the surah page: per-ayah recitation play buttons, full-surah murottal (Alafasy), a show/hide translation toggle, and a **Tajwid** mode with rule-color legend
- Continue-reading ("Terakhir Dibaca") cards on the home page driven by local reading history, recorded automatically as you open ayahs
- Tafsir chosen on the daily verse carries over to the ayah page (tab auto-selects the same tafsir)
...
- [Islamic Network audio CDN](https://cdn.islamic.network/) — recitation audio (full-surah murottal & per-ayah, Alafasy)
- [AlQuran Cloud API](https://alquran.cloud/) — `quran-tajweed` edition for colored tajwid text
...
Static content is generated by two scripts:

```bash
npm run data:prepare   # from raw JSON (surah index, quran, hadith, daily, ...)
npm run data:tajwid    # fetches the quran-tajweed edition from AlQuran Cloud
```
...
- `public/data/tajwid/:number.json` — per-surah tajweed text (lazy-loaded when Tajwid mode is enabled)
...
| `npm run data:tajwid` | Re-fetch tajweed text from AlQuran Cloud into `public/data/tajwid/` |
...
app/
  ...
  lib/
    ...
    tajweed.tsx     # tajwid text parser + rule-color map/legend
...
scripts/
  ...
  fetch-tajwid.mjs  # tajweed fetcher (AlQuran Cloud)
